...
func (a *API) createOrder(w http.ResponseWriter, r *http.Request) {
	...
	if err := dec.Decode(&in); err != nil {
		writeError(r.Context(), w, http.StatusBadRequest, codeInvalidJSON, "request body is not valid JSON for an order", "")
		return
	}
	...
	if err := in.Validate(); err != nil {
		...
		if errors.As(err, &ve) {
			writeError(r.Context(), w, http.StatusUnprocessableEntity, codeValidation, ve.Reason, ve.Field)
			return
		}
		writeError(r.Context(), w, http.StatusUnprocessableEntity, codeValidation, err.Error(), "")
		return
	}

	order := in.Build()
	if err := a.store.CreateOrder(r.Context(), order); err != nil {
		// logging.PII pseudonymises the customer identifier rather than
		// redacting it. Redaction would break the question you actually ask
		// during an incident — "show me everything that happened to this
		// customer" — while the raw value in a log store is exactly the kind of
		// personal data that turns a log breach into a reportable one. A stable
		// hash keeps the correlation and loses the identity.
		a.log.ErrorContext(r.Context(), "create order failed",
			"error", err,
			"currency", order.Currency,
			logging.PII("customer_id", order.CustomerID))
		writeError(r.Context(), w, http.StatusInternalServerError, codeInternal, "could not create order", "")
		return
	}
	...
	writeJSON(r.Context(), w, http.StatusCreated, order)
}
...
func (a *API) getOrder(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeError(r.Context(), w, http.StatusBadRequest, codeBadID, "order id must be a UUID", "id")
		return
	}
	...
	case errors.Is(err, domain.ErrNotFound):
		writeError(r.Context(), w, http.StatusNotFound, codeNotFound, "no order with that id", "")
		return
	case err != nil:
		a.log.ErrorContext(r.Context(), "get order failed", "error", err)
		writeError(r.Context(), w, http.StatusInternalServerError, codeInternal, "could not read order", "")
		return
	}
	writeJSON(r.Context(), w, http.StatusOK, order)
}
...
func (a *API) listOrders(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || limit < 1 || limit > 100 {
		writeError(r.Context(), w, http.StatusBadRequest, codeBadQuery, "limit must be an integer between 1 and 100", "limit")
		return
	}
	offset, err := intParam(q.Get("offset"), 0)
	if err != nil || offset < 0 {
		writeError(r.Context(), w, http.StatusBadRequest, codeBadQuery, "offset must be a non-negative integer", "offset")
		return
	}
	...
	if s := q.Get("status"); s != "" {
		...
		if !status.Valid() {
			writeError(r.Context(), w, http.StatusBadRequest, codeBadQuery, "unknown status", "status")
			return
		}
	}
	...
	if err != nil {
		...
		writeError(r.Context(), w, http.StatusInternalServerError, codeInternal, "could not list orders", "")
		return
	}
	writeJSON(r.Context(), w, http.StatusOK, listResponse{Orders: orders, Limit: limit, Offset: offset})
}
...
func (a *API) readyz(w http.ResponseWriter, r *http.Request) {
	if err := a.store.Ping(r.Context()); err != nil {
		...
		writeError(r.Context(), w, http.StatusServiceUnavailable, codeUnavailable, "database unreachable", "")
		return
	}
...
func (a *API) slowQuery(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || d < 0 || d > 5*time.Second {
		writeError(r.Context(), w, http.StatusBadRequest, codeBadQuery, "d must be a duration between 0 and 5s", "d")
		return
	}
	...
	if err := a.store.Sleep(r.Context(), d); err != nil {
		...
		if errors.Is(err, context.Canceled) || errors.Is(err, context.DeadlineExceeded) {
			writeError(r.Context(), w, http.StatusServiceUnavailable, codeUnavailable, "request abandoned while waiting for a connection", "")
			return
		}
		a.log.ErrorContext(r.Context(), "slow query failed", "error", err)
		writeError(r.Context(), w, http.StatusInternalServerError, codeInternal, "slow query failed", "")
		return
	}
	writeJSON(r.Context(), w, http.StatusOK, map[string]string{"slept": d.String()})
}
